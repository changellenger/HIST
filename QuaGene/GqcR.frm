VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} GqcR 
   OleObjectBlob   =   "GqcR.frx":0000
   Caption         =   "따라하기 : Xbar-R관리도"
   ClientHeight    =   5565
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   10800
   ...
   TypeInfoVer     =   39
End
Attribute VB_Name = "GqcR"
Attribute VB_Base = "0{F7ABBA55-7334-459D-BDEF-1F759A62EBB4}{AA38C284-7EB4-4BBA-94F6-92BEC43980A2}"
...
' no = Me.TextBox1.Value
    ' Rinterface.RRun "x1 <- matrix(data= arraytest, ncol= " & no & ", byrow = TRUE)"
    
    
     'Rinterface.RRun "xbar <- qcc(x1, type=" & Chr(34) & " xbar" & Chr(34) & ", nsigmas = 3,plot = FALSE)"
...
c = " r <- qcc(x1, type= " & Chr(34) & "R" & Chr(34) & ", nsigmas=" & no2 & ", title = " & Chr(34) & "R 관리도" & Chr(34) & ")"
...
Range(Cells(stn + 37, 3), Cells(stn + 37, 3)).Value = "R관리도 결과해석"
...
Range(Cells(stn + 39, 3), Cells(stn + 39, 3)).Value = "R관리상한선을 벗어나는 부분군:"
...
Range(Cells(stn + 41, 3), Cells(stn + 41, 3)).Value = "R관리하한선을 벗어나는 부분군:"
...
'If Range("b41").Value & Range("b43").Value = "" Then
   ' Range("b52").Value = "공정이 관리상태에 있는 것으로 판정할 수 있습니다."
'ElseIf Range("b41").Value <> "" Then
 '   Range("b42").Value = "번째 부분군이 '관리상한선'을 벗어났습니다. 따라서 공정에 이상원인이 있는 것으로 추정됩니다."
  '  End If

'If Range("b41").Value & Range("b43").Value = "" Then
'ElseIf Range("b43").Value <> "" Then
'Range("b44").Value = "번째 부분군이 '관리하한선'을 벗어났습니다. 따라서 공정에 이상원인이 있는 것으로 추정됩니다."
'End If
...
'If Range("o3").Value & Range("o5").Value = "" Then
'    Range("o7").Value = "공정이 관리상태에 있는 것으로 판정할 수 있습니다."
'ElseIf Range("o3").Value <> "" Then
'    Range("o4").Value = "번째 부분군이 '관리상한선'을 벗어났습니다. 따라서 공정에 이상원인이 있는 것으로 추정됩니다."
'ElseIf Range("o5").Value = "" Then
'Range("o6").Value = "번째 부분군이 '관리하한선'을 벗어났습니다. 따라서 공정에 이상원인이 있는 것으로 추정됩니다."
'End If
...
'If Range("b48").Value & Range("b50").Value = "" Then
 '   Range("b52").Value = "공정이 관리상태에 있는 것으로 판정할 수 있습니다."
'ElseIf Range("b48").Value <> "" Then
 '   Range("b49").Value = "번째 부분군이 '관리상한선'을 벗어났습니다. 따라서 공정에 이상원인이 있는 것으로 추정됩니다."
  '  End If
    
'If Range("b48").Value & Range("b50").Value = "" Then
'ElseIf Range("b50").Value <> "" Then
'Range("b51").Value = "번째 부분군이 '관리하한선'을 벗어났습니다. 따라서 공정에 이상원인이 있는 것으로 추정됩니다."
'End If
...
Dim btnR As String
btnR = Chr(39) & "btnR ""( " & no & ")" & Chr(39)


Module2.makebtn btnR
