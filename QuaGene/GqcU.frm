VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} GqcU 
   OleObjectBlob   =   "GqcU.frx":0000
   Caption         =   "따라하기 : U관리도"
   ClientHeight    =   5445
   ClientLeft      =   45
   ClientTop       =   390
   ...
   TypeInfoVer     =   24
End
Attribute VB_Name = "GqcU"
Attribute VB_Base = "0{C84BEE97-52AE-4FFE-BF77-7B5814E518C0}{B40B9612-6508-42F2-8F32-54FEE248B4EF}"
...
'Rinterface.RRun "qcc(x1, type= " & Chr(34) & "u" & Chr(34) & ", size=5)"
...
a = " u <- qcc(arraytest, type= " & Chr(34) & "u" & Chr(34) & ", size= arraytest2, title = " & Chr(34) & "U 관리도" & Chr(34) & ")"
...
Range(Cells(stn + 30, 4), Cells(stn + 30, 4)).Value = "U관리도 결과해석"
...
Range(Cells(stn + 32, 4), Cells(stn + 32, 4)).Value = "U관리상한선을 벗어나는 부분군:"
...
Rinterface.RRun "SD <- sum(arraytest)" '총 결점수'
...
Range(Cells(stn + 6, 8), Cells(stn + 6, 8)).Value = "총 단위수"
Range(Cells(stn + 6, 8), Cells(stn + 6, 8)).Cells.ColumnWidth = 15
Range(Cells(stn + 7, 8), Cells(stn + 7, 8)).Value = "총 결점수"
Range(Cells(stn + 7, 8), Cells(stn + 7, 8)).Cells.ColumnWidth = 15
Range(Cells(stn + 8, 8), Cells(stn + 8, 8)).Value = "단위당 결점 수"
...
Rinterface.GetArray "SNMSG", Range(Cells(stn + 6, 9), Cells(stn + 6, 9))
Rinterface.GetArray "SD", Range(Cells(stn + 7, 9), Cells(stn + 7, 9))
